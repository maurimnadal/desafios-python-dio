...
class ContaCorrente(Conta):
    ...
    @classmethod
    def nova_conta(cls, usuario, numero, limite, limite_saques):
...
def log_transacao(func):
    def envelope(*args, **kwargs):
        ...
        print(f"{datetime.strftime(datetime.now(), "%d/%m/%Y %H:%M")} - Operação: {func.__name__.upper()}")
        return resultado
...
def criar_conta(numero_conta, usuarios, contas):
    ...
    conta = ContaCorrente.nova_conta(
        usuario=usuario, numero=numero_conta, limite=500, limite_saques=50
    )
    contas.append(conta)
